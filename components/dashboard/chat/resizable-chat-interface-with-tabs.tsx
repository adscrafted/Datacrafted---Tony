...
import { ChatInterfaceWithTabs } from './chat-interface-with-tabs'
import { useDashboardTab } from '@/lib/contexts/dashboard-tab-context'

interface ResizableChatInterfaceWithTabsProps {
  minWidth?: number
  maxWidth?: number
  defaultWidth?: number
...
export function ResizableChatInterfaceWithTabs({
  minWidth = 320,
  maxWidth = 600,
  defaultWidth = 384,
  onWidthChange
}: ResizableChatInterfaceWithTabsProps) {
  const { activeTabId, tabAnalyses, setTabAnalyses } = useDashboardTab()
  ...
  return (
    ...
    >
      ...
      {/* Chat Interface with Tab Support */}
      <div className="flex-1 overflow-hidden">
        <ChatInterfaceWithTabs 
          activeTabId={activeTabId}
          tabAnalyses={tabAnalyses}
          setTabAnalyses={setTabAnalyses}
        />
      </div>
    </div>
  )
